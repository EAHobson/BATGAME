...
```{r logo, echo=FALSE, fig.cap="For more information, visit http://hobsonresearch.com/index.php/sociality-game/", out.width = '100%'}
...
## More vampire bat information & research
...
Load landmark locations: We also load the landmarks data which we will use for plotting and for some of our distance analyses.
```{r}
...
## **Q1: Importing data
**What is the text "#load landmark locations" on line 116? Why is it a different color?**
```
...
explore.locs <- read_csv("example_explore.csv") #this is uploaded by the students
...
Load data & process
```{r}
rand.locs <- read_csv("example_random.csv") #this is uploaded by the students
...
#straightness
player.distance <- player$value[player$type.mm=="distance"]
distance <- ggplot(NPCs[NPCs$type.mm=="distance",], aes(type.mm, value)) +
...
gridExtra::grid.arrange(distance, straightness, dirchange, ncol=3,
...
We are now going to play another version of the game. You'll now choose "Social Preferences" as the movement type (https://eahobson.github.io). In the "Social Preferences" version of the game, the NPC bats have new rules that they use to structure their movements. Each NPC bat is assigned a "buddy". 
...
Load data & process
```{r warning=FALSE}
socfor.locs <- read_csv("example_socforag.csv") #this is uploaded by the students #SOCFORG.csv
...
Summarize all trajectories: we'll use the same method we used earlier to measure two aspects of your trajectories, and those of the other bats: staightness of the path and the mean number of direction changes.
```{r}
#make empty dataframe to write trajectory summaries to
trajectory.summary <- data.frame(batID=character(),
                                 distance=double(),
...
# plot all together to compare
gridExtra::grid.arrange(distance, straightness, dirchange, ncol=3,
                        top=textGrob("Bat trajectories: social preferences"))
...
ggplot(unique.landmarks_cows, aes(x=batID, y=unique.visitsXtype, 
  ...
  labs(x="Bats", 
       y="Number of cows found", 
       title = "Bat trajectories: social preferences") +
  ## add manual color scale
  scale_fill_manual("legend",         ###### in this call: colour change
                    values = c("Player" = "red", "NPC bats" = "saddlebrown")) +
  theme(plot.title = element_text(hjust = 0.5),
...
## ** Q13: Social preferences foraging success
...
```
**Is social preferences foraging potentially a successful foraging strategy that you'd expect real bats to use? Why or why not?**
```
ADD YOUR ANSWER HERE
...
**For social preferences foraging, compare your social measures to the NPC bats. Were you behaving "normally" compared to the other bats? If not, how did your movements cause you to have different social measures than the NPC bats?**
```
ADD YOUR ANSWER HERE
...
plot(g, 
     ...
     main= "Social preferences proximity network (all near events)", 
...
plot(g2, 
     ...
     edge.color=alpha("black", 0.4),
     main= "Bat trajectories: thresholded social preferences (rare observations removed)", 
...
top.assos <- ggplot(top.associate, aes(x=batB, y=n.near, group=1, fill = ifelse(batA == "Player", "Player", "NPC bats"))) +
  ...
  labs(title="Stongest-preferred associates", 
       subtitle= "Bat trajectories: social preferences",
       x="Bats", 
...
under <- ggplot(player.locs, aes(x, y)) +
  ...
  xlim(0,50) +
  ylim(50,0) +
  labs(title = "Bat trajectories: social preferences, top associates of Player",
...
Load data & process
```{r warning=FALSE}
bet_hedge.locs <- read_csv("example_bethedge.csv") #this is uploaded by the students #SOCFORG.csv
...
```{r}
   ...
   #clean landmarks
    ...
    for (step in 2:length(ev10steps)) { #step=1
      ...
      #if no data at that timestep, skip
      loop.data <- if(nrow(subset(bet_hedge.locs, times==loop.step))<1) next #skip if no data at this ts
      
      #otherwise, subset the data
      loop.data <- subset(bet_hedge.locs, times==loop.step)
...
ggplot(unique.landmarks_cows, aes(x=batID, y=unique.visitsXtype, 
  ...
  labs(x="Bats", 
       y="Number of cows found", 
       title = "Bat trajectories: bet hedging") +
  ## add manual color scale
  scale_fill_manual("legend",         ###### in this call: colour change
                    values = c("Player" = "red", "NPC bats" = "saddlebrown")) +
  theme(plot.title = element_text(hjust = 0.5),
...
all.bats <- as.data.frame(unique(bet_hedge.locs$batID))
colnames(all.bats) <- "batID"
...
# plot all together to compare
gridExtra::grid.arrange(plot_degree, plot_strength, ncol=2,
                        top=textGrob("Bat trajectories: bet hedging"))
```
...
plot(g2, 
     edge.width=E(g2)$n.near/3,
     edge.color=alpha("black", 0.4),
     main= "Bat trajectories: bet hedging", 
...
```{r socforg, echo=FALSE, fig.cap="For more information, visit https://doi.org/10.1371/journal.pbio.3001366", out.width = '100%'}
knitr::include_graphics("socforag.scenarios.png")
```
...
**Scenario A: Preferred roostmates do not coordinate movements outside the roost while foraging.** 

**What rules should the player and the NPC bats follow in a game like Scenario A?**
```
ADD YOUR ANSWER HERE
```

**If you were a researcher in the field studying these bats, how could you tell whether bats were following Scenario A? What would you look for? How would you tell which bats preferred each other?**
```
ADD YOUR ANSWER HERE
```

**Scenario B: Preferred roostmates coordinate their departures from the roost but avoid foraging with each other** 

**What rules should the player and the NPC bats follow in a game like Scenario B?**
```
ADD YOUR ANSWER HERE
```

**If you were a researcher in the field studying these bats, how could you tell whether bats were following Scenario B? What would you look for? How would you tell which bats preferred each other?**
```
ADD YOUR ANSWER HERE
```

**Scenario F: All roostmates depart and forage as a group.**

**What rules should the player and the NPC bats follow in a game like Scenario F?**

```
ADD YOUR ANSWER HERE
```

**If you were a researcher in the field studying these bats, how could you tell whether bats were following Scenario F? What would you look for? How would you tell which bats preferred each other?**
```
ADD YOUR ANSWER HERE
```
